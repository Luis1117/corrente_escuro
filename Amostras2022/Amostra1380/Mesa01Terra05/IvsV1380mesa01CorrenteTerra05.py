...
def my_plot_EavsV():
    ...
    for i in range(101):
        ...
        lr_Ln = np.polyfit(Ea_V[i, :][9:20, 10],
                           Ea_V[i, :][9:20, 8], 1, cov=True)
    ...
    Ea1380 = np.c_[dados, dados[:, 0]*8.61e-5, dados[:, 1]
    ...
    plt.title('Amostra 1380 (paso voltagem = 0.1V) \n''Corrente#01-Terra#05')
    # plt.scatter(x_EA, y_EA, yerr=Err_Y, fmt='o')
    plt.grid(axis='both')
    plt.xlabel('Tensão (V)')
    plt.ylabel('Ea (meV)')
    plt.errorbar(x_EA, y_EA, yerr=Err_Y, fmt='o', color='r')
    plt.savefig('EaCorrente#01-Terra#05.jpg', dpi=300)

    # np.savetxt('Ea1380Mesa#01_Terra05.txt', Ea1380, delimiter=',')
    return plt.show()
