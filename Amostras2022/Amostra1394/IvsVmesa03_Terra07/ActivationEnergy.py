...
# c = np.around(b[:, 0], 2)
# print(np.searchsorted(c, np.arange(-5, 6, 1)))
d = glob.glob('./*.txt')
# print(len(d))
m_T = []
...
# print(dl)
LnT = np.c_[dl, 1/(dl[:,0]), dl[:,1]/(dl[:,0]**2), (1/dl[:,0])*5e-8, dl[:,1]/((dl[:,0])*(10e-8)), np.log((1/dl[:,0])*(5e-8)), dl[:,1]/(dl[:,0]), 1/(dl[:,3] )]
# print(LnT[0])
LnTV=np.reshape(LnT, (22,11,11))
# print(LnTV[:, 0])
Ea = []
for i in range(11):
...
# # Dominio de cada plot:
x_5V=Ea_V[0, :][12:22, 10]
y_5V=Ea_V[0, :][12:22, 8]

x_4V=Ea_V[1, :][12:22, 10]
y_4V=Ea_V[1, :][12:22, 8]

x_3V=Ea_V[2, :][12:22, 10]
y_3V=Ea_V[2, :][12:22, 8]

x_2V=Ea_V[3, :][12:22, 10]
y_2V=Ea_V[3, :][12:22, 8]

x_1V=Ea_V[4, :][12:22, 10]
y_1V=Ea_V[4, :][12:22, 8]
...
x5V=Ea_V[10, :][12:22, 10]
y5V=Ea_V[10, :][12:22, 8]

# Plot das figuras Ln vs 1/T:
...
# plt.title('Amostra 1394 Mesa#03-Terra#07')
# plt.show()


axisX=Ea_V[1, :][10:22, 10]
axisY=Ea_V[1, :][10:22, 8]

# pendiente = np.polyfit(axisX, axisY, 1)
# print(pendiente)
...
for i in range(11):
    ...
    lr_Ln=np.polyfit(Ea_V[i, :][13:22, 10], Ea_V[i, :][13:22, 8], 1, cov=True)
...
# print(dados)
Ea1394=np.c_[dados, dados[:, 0]*8.61e-5, dados[:, 1]*8.61e-5, dados[:, 0]*8.61e-2,  dados[:, 1]*8.61e-2  ]
# print(Ea1394)
...
# plt.title('Amostra 1394 Mesa#03-Terra07')
...
np.savetxt('Ea1394Mesa#3_Terra07.txt', Ea1394, delimiter=',')