...
def my_plot_IvsV():
    dadosiniciais = sorted(glob.glob('./*K_hit.txt'))
    renome01 = np.char.replace(np.array(dadosiniciais), '_hit.txt', '')
...
def my_plot_EavsV():
    ...
    d = glob.glob('./*K_hit.txt')  # List of txt.
    ...
    plt.title('Amostra 1394 Mesa#04-Terra07 (paso voltagem = 0.1V)')
    ...
    np.savetxt('Ea1394Mesa#4_Terra07MelhorRes.txt', Ea1394, delimiter=',')
    return plt.show()
