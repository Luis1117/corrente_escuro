...
a = np.genfromtxt('novo80K.txt', delimiter=',', dtype=float)
b = np.reshape(a, (501, 5))

d = np.genfromtxt('novo80K.txt', delimiter=',', dtype=float)
e = np.reshape(d, (501, 5))

plt.scatter(b[:, 0], b[:, 1], label='1')
plt.scatter(e[:, 0], e[:, 1], label='2')
plt.legend()
plt.show()
